...
function getRoundOverText() {
    if ((jsPsych.totalTime()>endTime))  {
        return '<div class = centerbox><p class = center-block-text>' + round_over_text +
        ' Die Zeit ist um :( <p class = center-block-text>Drücken Sie <strong>enter</strong> um zum zu Beenden.</p></div>'
    }else 
    return '<div class = centerbox><p class = center-block-text>' + round_over_text +
...
/*
SC: THIS DEFINES THE HTML OF THE GAME
*/
function getGame() {
   ...
   //check if timeout is reached
    ...
    game_state = appendTextAfter(game_state, 'Gefangene Fische: </strong>', trip_bank)
    ...
    if (num_fish_curr_pond > 0) {
        main_timeout = setTimeout(() => {
            ...
            $('.lake').on('click', '.redfish', function() {
                // Code to execute when a redfish is clicked
                console.log('A fish was clicked!');
                goFish(true);
                $(this).remove(); // Removes the clicked redfish
                jsPsych.finishTrial();
            });
...
var fishWaitingTime = function () {
    /*we set a time to wait for new fish*/
...
function makeFish(fish_num) {
    ...
    */
    ...
    /*Instead of binding directly to .redfish, you can use event delegation to attach the click event 
    to the parent container (.lake) and listen for clicks on the fish elements within it.*/
    place_fish()
}
...
var num_fish_in_ponds_base = [17, 2, 8, 1, 4, 11, 6, 5, 6, 2, 6, 4, 5, 6, 6, 2, 17, 3, 8, 3, 8, 23, 5, 13, 7, 3, 8, 7, 2, 5, 8,
    ...
    4, 10, 2, 15, 8, 9, 20, 4, 8, 15, 8, 9, 3, 6, 12, 3, 5, 3, 6, 2, 1, 14, 7, 4, 15, 4, 7, 5, 5, 2,
    ...
    8, 10, 9, 5, 1, 5, 7, 17, 3, 6, 6, 6, 20, 6, 9, 7, 4, 11, 9, 11, 9, 7, 12, 6, 1, 13, 5, 8, 7, 10,
    8, 11, 2, 4, 8, 6, 12, 1, 19, 9, 11, 4, 5, 16, 24, 9, 8, 5, 13, 11, 7, 20, 9, 7, 4, 9, 3, 2, 2, 1,
    ...
    2, 10, 5, 17, 3, 1, 9, 8, 4, 6, 5, 11, 18, 11, 5, 10, 6, 10, 7, 7, 3, 5, 13, 12, 5, 5, 8, 15, 5, 11,
    ...
    6, 7, 7, 7, 1, 6, 5, 8, 11, 11, 4, 7, 6, 10, 11, 8, 5, 6, 10, 3, 5, 12, 4, 10, 1, 4, 11, 13, 2, 17,
    7, 6, 16, 9, 7, 11, 3, 14, 6, 6, 12, 10, 14, 2, 11, 8, 4, 10, 3, 4, 8, 4, 5, 8, 1, 2, 13, 6, 4, 6,
    6, 8, 9, 14, 4, 4, 13, 9, 15, 2, 2, 4, 9, 1, 8, 13, 6, 10, 4, 3, 6, 6, 3, 9, 8, 12, 2, 13, 7, 8,
    ...
    7, 9, 13, 12, 1, 8, 8, 6, 5, 3, 12, 6, 7, 7, 6, 1, 13, 5, 5, 8, 8, 2, 5, 6, 10, 6, 11, 3, 10, 14,
    14, 7, 6, 5, 4, 16, 11, 14, 4, 3, 2, 11, 8, 1, 2, 12, 0, 9, 3, 10, 1, 10, 4, 7, 4, 11, 5, 15, 7, 13,
    ...
    1, 3, 6, 3, 4, 4, 17, 8, 4, 8, 8, 10, 7, 4, 13, 13, 9, 6, 8, 5, 11, 4, 1, 4, 10, 5, 8, 10, 7, 5,
    9, 5, 4, 10, 4, 16, 12, 7, 9, 5, 2, 4, 13, 4, 5, 3, 11, 5, 1, 3, 14, 9, 11, 8, 15, 11, 3, 7, 14, 11,
    8, 7, 4, 5, 2, 11, 14, 2, 12, 10, 2, 5, 3, 8, 2, 5, 17, 6, 5, 7, 6, 10, 1, 7, 8, 3, 7, 14, 7, 4,
    ...
    9, 7, 9, 1, 9, 11, 5, 9, 11, 5, 8, 4, 1, 1, 4, 10, 6, 1, 8, 3, 9, 8, 7, 2, 3, 14, 6, 2, 8, 5,
...
var game_setup = "<div class = titlebox><div class = center-text></div></div>" +
    "<div class = lake></div>" +
    ...
    "<button id='Collect' class='select-button' onclick='collect()' style='padding: 12px 30px; min-width: 150px;'>Teich wechseln</button>"+
    "</div>" +
    "<div class = subimgcontainer>" +
    "<div class = imgbox></div>" +
    "</div>" +
    "<div class = subinfocontainer>" +
    "<div class = infobox><p class = info-text id = trip_bank><strong> Gefangene Fische: </strong></p></div> " +
    //"<div class = infobox><p class = info-text id = tournament_bank><strong>Bonuspunkte Gesamt: </strong></p></div>" +
    "</div>" +
    "</div>" +
    "</div><div class = buttonbox>"+//<button id = 'goFish' class = select-button onclick = goFish(true) disabled>Fischen</button>"+
    "<button id = 'hiddenTrigger' class = select-button onclick = goFish(false) hidden style='display: none;'></button></div>"
    "<button id='hiddenCollect' class='select-button' onclick='collect()' style='display: none;'></button>"
...
var game_node = {
    ...
    loop_function: function (data) {
        //continue if you did not click and time is not up
        if ((round_over == 1) || (jsPsych.totalTime()>endTime))  {
            return false
        } else {
            return true
        }
...
var inner_game_loop = {
    timeline: [game_node,round_over_block],
    loop_function: function (data) {
        if (jsPsych.totalTime() > endTime) {
...
angling_risk_task_experiment.push(intro)
angling_risk_task_experiment.push(inner_game_loop)
